...
export default function APIPageClient({ machineId }) {
  ...
  // Security gate: block remote exposure while dashboard sign-in is disabled or no local credential exists.
  const hasCredentialProtection = hasPassword || hasUsers;
  const isLoginUnsafe = !requireLogin || !hasCredentialProtection;
  const unsafeReason = !requireLogin
    ? "Enable \"Require login\" and keep dashboard sign-in protected before activating the tunnel."
    : "Create a dashboard account or set a custom dashboard password before activating the tunnel.";
      ...
      <Card>
        ...
        {(tunnelEnabled || tsEnabled) && (
          <div className="mt-4 flex flex-col gap-2">
            ...
            {(!requireLogin || !hasCredentialProtection) && (
              <SecurityWarning
                message={
                  !requireLogin
                    ? "Require login is disabled — anyone can access your dashboard via tunnel."
                    : "Dashboard does not have a configured local credential yet — set one in Profile settings."
                }
                action={{
                  label: !requireLogin ? "Enable" : "Open profile",
